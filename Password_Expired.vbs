...
strEmailFrom = "<email>"
strEmailSubject = "Password Expiration / Caducidad de Contrase–a / Vencimento de senha"
strSMTP = "smtp-server"
...
Do Until objRecordSet.EOF
    ...
    'WScript.Echo whenPasswordExpires
		...
		strEmailBody = strEmailBody & "===============================================" & vbcrlf
		strEmailBody = strEmailBody & " " & vbcrlf
		strEmailBody = strEmailBody & "Atención: su contrase–a del dominio Company caducar‡ en " & whenPasswordExpires & " días." & vbcrlf
		strEmailBody = strEmailBody & "Por favor, c‡mbiela para evitar problemas de acceso al sistema." & vbcrlf
		strEmailBody = strEmailBody & "Reciba un cordial saludo" & vbcrlf
		strEmailBody = strEmailBody & " " & vbcrlf
		strEmailBody = strEmailBody & "===============================================" & vbcrlf
		strEmailBody = strEmailBody & " " & vbcrlf
		strEmailBody = strEmailBody & "Atenao: a sua senha de acesso ao dominio Company vencera em " & whenPasswordExpires & " d’as." & vbcrlf
